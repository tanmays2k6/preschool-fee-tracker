...
async function updatePlaygroupDetails() {
  console.log('Updating Playgroup student details (Mother names, DOB, etc.)...');

  for (const student of pgStudentsToUpdate) {
    ...
    if (existing) {
      ...
      if (error) {
        ...
        console.log(`✅ Updated ${student.admission_no} (${student.name}) - Mother: ${student.mother_name}`);
      }
    } else {
      const { data, error } = await supabase
        .from('students')
      ...
      if (error) {
        ...
        console.log(`✅ Inserted ${student.admission_no} (${student.name}) - Mother: ${student.mother_name}`);
      }
    }
  }

  console.log('Finished updating Playgroup details.');
}

updatePlaygroupDetails();
